        ...
        <Tab.List className="flex space-x-10 rounded-xl p-1 backdrop-blur-md bg-slate-50 dark:bg-slate-800 bg-opacity-30 dark:bg-opacity-30">
          {categories.map((tab) => (
            <a href={tab.href} key={tab.label}>
              <Tab
                className={({ selected }) =>
                  `w-full backdrop-blur-md bg-opacity-25 rounded-lg px-4 py-2.5 text-sm font-medium leading-5 text-blue-700 dark:text-blue-400 ring-white ring-opacity-60 ring-offset-2 ring-offset-blue-400 focus:outline-none focus:ring-2 ${selected ? "dark:bg-opacity-50 bg-white shadow dark:bg-slate-800" : "dark:bg-opacity-10 text-blue-500 bg-slate-200 dark:bg-slate-700 hover:bg-white/[0.12] hover:text-white}"}`
                }
              >
                {tab.label}
              </Tab>
    ...
    <nav className="bg-zinc-200 px-2 sm:px-4 py-2.5 dark:bg-zinc-800 w-full z-20 top-0 left-0 border-b border-slate-200 dark:border-slate-600 sticky dark:bg-opacity-50 bg-opacity-50 backdrop-blur-[8px] rounded-3xl">
      <div className="flex flex-wrap items-center justify-between ml-4 max-sm:ml-1">
        ...
        <div className="flex order-last space-x-1 ">
          ...
          <Link
            type="button"
            className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-bold rounded-lg text-sm px-5 max-md:px-2 pt-3.5 pb-2 text-center mr-3 md:mr-0 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 hidden lg:block h-12"
            href="/register"
            draggable={false}
          >
            Registre-se
          ...
          <Link
            type="button"
            className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-bold rounded-lg text-sm p-3 my-0.5 text-center mr-3 md:mr-0 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 block lg:hidden h-12 aspect-1"
            href="/register"
            draggable={false}
          >
            <svg
