...
function genGrid(
  ...
  return Array(max_tries).fill(
    Array(word_length).fill([
      "",
      ...(Array<LetterStatus>(grids_count).fill(
        LetterStatus.UNTESTED,
      ) as NonEmptyArray<LetterStatus>),
    ]),
  );
}
...
export default function Termo({
  ...
  function executeTry(
    ...
    const _word_with_accent: string | undefined =
      _status.available_words[payload.attempt];
    ...
    for (const res of w_try)
      statuses.push([
        res,
        ...(Array<LetterStatus>(gridLength).fill(
          LetterStatus.UNTESTED,
        ) as NonEmptyArray<LetterStatus>),
      ]);
    ...
    for (const [gridIndex, res] of _status.final_word.entries()) {
      ...
      for (let i = 0; i < _status.WORD_LENGTH; i++) {
        ...
        if (typeof n === "undefined") letters.set(letter, 1);
    ...
    for (const [i, l] of word_with_accent.entries()) statuses[i][0] = l;
    ...
    for (let i = 1; i <= _status.final_word.length; i++) {
      ...
      if (won) _status.won[i - 1] = [true, _status.round_num];
    }
  ...
  const [data, gridDispatch] = useReducer(
    GridReducer,
    JSON.stringify({
      grid: genGrid(
      ...
      available_words,
      round_num: 0,
      won: Array<[boolean, number]>(final_word.length).fill([
        false,
        -1,
      ]) as NonEmptyArray<[boolean, number]>,
      available_letters: Array<Record<Letter, LetterStatus>>(
        final_word.length,
      ).fill({
        a: LetterStatus.UNTESTED,
        b: LetterStatus.UNTESTED,
        c: LetterStatus.UNTESTED,
        d: LetterStatus.UNTESTED,
        e: LetterStatus.UNTESTED,
        f: LetterStatus.UNTESTED,
        g: LetterStatus.UNTESTED,
        h: LetterStatus.UNTESTED,
        i: LetterStatus.UNTESTED,
        j: LetterStatus.UNTESTED,
        k: LetterStatus.UNTESTED,
        l: LetterStatus.UNTESTED,
        m: LetterStatus.UNTESTED,
        n: LetterStatus.UNTESTED,
        o: LetterStatus.UNTESTED,
        p: LetterStatus.UNTESTED,
        q: LetterStatus.UNTESTED,
        r: LetterStatus.UNTESTED,
        s: LetterStatus.UNTESTED,
        t: LetterStatus.UNTESTED,
        u: LetterStatus.UNTESTED,
        v: LetterStatus.UNTESTED,
        w: LetterStatus.UNTESTED,
        x: LetterStatus.UNTESTED,
        y: LetterStatus.UNTESTED,
        z: LetterStatus.UNTESTED,
      }),
      error: null,
  ...
  const canvasRefs: React.MutableRefObject<
    HTMLCanvasElement & { confetti?: ReturnType<typeof create> }
  >[] = (() => {
    const a = [];
  ...
  const gotAllRight =
    globalStatus.won.filter((g) => g[0] === false).length === 0;
  const gameFinished =
    globalStatus.round_num >= globalStatus.MAX_TRIES || gotAllRight;
    ...
    if (globalStatus.error !== null) {
      ...
      document
        .querySelector<HTMLInputElement>(
          `[item-input=INPUT-${globalStatus.round_num}-0]`,
        )
        ?.focus();
          ...
          setTimeout(
            () => {
              for (const el of document.querySelectorAll(
                `[item-card=CARD-${globalStatus.round_num - 1}-${i}]`,
              ))
    ...
    <div className="flex flex-col items-center">
                        ...
                        >
                          {row.map((el, l) => {
                            ...
                            const right_position =
                              "dark:bg-lime-600 bg-lime-500 text-white";
                            const right_word =
                              "dark:bg-yellow-500 bg-yellow-400 text-white";
                            const wrong_letter =
                              "dark:bg-zinc-700 bg-zinc-400 text-white";
                            const enabled =
                              "dark:border-cyan-800 border-cyan-200 focus-within:border-cyan-500 border-2";
                            ...
                            function previous<P extends Element>(
                              loopOver = true,
                            ) {
                              if (l === 0 && loopOver)
                                return document.querySelector<P>(
                            ...
                            return (
                              <div
                                ...
                                item-card={
                                  globalStatus.won[gridIndex][0] &&
                                  i > globalStatus.won[gridIndex][1]
                                    ? undefined
                                    : `CARD-${i}-${l}`
                                }
                              >
                                <div className={styles.letterInner}>
                                  ...
                                  <div
                                    className={`${status} w-full h-full rounded-lg ${styles.letterBack}`}
                                  >
                                    <span className="text-center font-extrabolt text-3xl uppercase">
                                      {el[0]}
                                    </span>
                                  </div>
                                </div>
      ...
      <div className="flex flex-col gap-2 mt-8 items-center justify-center">
        <div className="grid grid-rows-1 grid-cols-10 gap-2 max-w-fit">
          {(["q", "w", "e", "r", "t", "y", "u", "i", "o", "p"] as Letter[]).map(
            (key) => (
              <LetterKey
                key={key}
                keyValue={key}
                available_letters={globalStatus.available_letters}
              />
            ),
          )}
        </div>
        <div className="grid grid-rows-1 grid-cols-9 gap-2 max-w-fit">
          {(["a", "s", "d", "f", "g", "h", "j", "k", "l"] as Letter[]).map(
            (key) => (
              <LetterKey
                key={key}
                keyValue={key}
        ...
        <div className="grid grid-rows-1 grid-cols-7 gap-2 max-w-fit">
          {(["z", "x", "c", "v", "b", "n", "m"] as Letter[]).map((key) => (
            <LetterKey
              key={key}
              keyValue={key}
              available_letters={globalStatus.available_letters}
  ...
  return (
    <>
      <Button onPress={onOpen} isIconOnly>
        <InfoIcon className="w-8 h-8 p-1 dark:fill-white fill-black" />
      </Button>
        ...
        <ModalContent>
          {(onClose) => (
            <>
              ...
              <ModalBody>
                <p>
                  Esta versão do Termo foi criada por mim, Arthur Bufalo, e foi
                  grandemente inspirada pelo trabalho do{" "}
                  <Link isExternal showAnchorIcon href="https://fserb.com/">
                    Fernando Seboncini
                  </Link>{" "}
                  e seu jogo{" "}
                  <Link isExternal showAnchorIcon href="https://term.ooo/">
                    Termooo
                  </Link>
                  .<br />
                  Apesar de ser bem similar, minha intenção não é lucrar em cima
                  deste projeto, nem defamar o criador original.
                  <br />
                  Este projeto foi criado com intenções de aprendizado e
                  diversão, e espero que você se divirta jogando tanto quanto eu
                  me diverti criando!
                </p>
                <p>
                  As palavras usadas nesse modelo foram retiradas das seguintes
                  fontes:
                  <ul className="list-disc list-insidelist-disc list-inside ml-2">
                    <li>
                      <Link
                        isExternal
                        showAnchorIcon
                        href="https://www.ime.usp.br/~pf/dicios/"
                      >
                        IME da USP
                      </Link>
                      , disponivel na licença{" "}
                      <Link
                        isExternal
                        showAnchorIcon
                        href="https://www.gnu.org/licenses/old-licenses/gpl-1.0.html"
                      >
                        GPL
                      </Link>
                    </li>
                    <li>
                      <Link
                        isExternal
                        showAnchorIcon
                        href="https://github.com/fserb/pt-br"
                      >
                        Léxico PT-BR
                      </Link>
                      , disponível na licença{" "}
                      <Link
                        isExternal
                        showAnchorIcon
                        href="https://raw.githubusercontent.com/fserb/pt-br/master/LICENSE"
                      >
                        MIT
                      </Link>
                    </li>
