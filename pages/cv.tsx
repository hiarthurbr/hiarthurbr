...
const CV = () => {
  ...
  const stop_in = useMemo(() => {
    return Math.round(
      90 / ((duration * keyframes.length) / (duration * (page + 1))),
    );
  }, [page]);
  const update = useCallback(
    (a: AnimeInstance) => {
      ...
      if (orb && section) {
        ...
        const stop = (() => {
          if (a.reversed) return Math.round(a.progress) <= stop_in;
          return Math.round(a.progress) >= stop_in;
        })();
        console.log(
          Math.round(a.progress),
          a.reversed ? "less" : "more",
          "or equal to",
          stop_in,
          stop,
        );
  ...
  return (
    <div className="w-full max-w-7xl max-xl:max-w-2xl max-sm:px-0 max-xl:px-8 max-sm:max-w-full select-none flex flex-row max-xl:flex-col justify-center items-center gap-10 dark:bg-slate-900 bg-slate-300 max-xl:rounded-xl rounded-2xl shadow-2xl bg-opacity-75 backdrop-blur-sm p-16 transition-all duration-500">
      <div className="relative w-fit h-fit">
        ...
        <div
          id="orb"
          className="w-96 h-96 rounded-full blur-md z-0"
          style={{
            backgroundImage:
              "linear-gradient(180deg, #FA05F0 0%, #AF00FA 100%)",
          }}
        />
      ...
      <div className="h-full w-full mx-8 xl:self-start">
        <ul className="mb-4 -mt-8 py-4 px-5 max-sm:px-0 flex flex-row max-sm:grid max-sm:grid-flow-row max-sm:grid-cols-3 max-sm:gap-6 capitalize dark:bg-slate-700 bg-slate-400 bg-opacity-50 dark:bg-opacity-75 backdrop-blur-sm rounded-lg justify-evenly">
          <li
            ...
            onClick={() => setPage(CV_Page.About)}
            onKeyDown={() => setPage(CV_Page.About)}
          >
            <a
              className="dark:text-white dark:hover:text-slate-300 hover:text-slate-700 text-slate-900 cursor-pointer font-bold"
              href={void 0}
            >
              About
            </a>
          ...
          <li
            className="hover:-translate-y-1 transition-all duration-500"
            onClick={() => setPage(CV_Page.Experience)}
            onKeyDown={() => setPage(CV_Page.Experience)}
          >
            <a
              className="dark:text-white dark:hover:text-slate-300 hover:text-slate-700 text-slate-900 cursor-pointer font-bold"
              href={void 0}
          ...
          <li
            className="hover:-translate-y-1 transition-all duration-500"
            onClick={() => setPage(CV_Page.Education)}
            onKeyDown={() => setPage(CV_Page.Education)}
          >
            <a
              className="dark:text-white dark:hover:text-slate-300 hover:text-slate-700 text-slate-900 cursor-pointer font-bold"
              href={void 0}
          ...
          <li
            className="hover:-translate-y-1 transition-all duration-500"
            onClick={() => setPage(CV_Page.Skills)}
            onKeyDown={() => setPage(CV_Page.Skills)}
          >
            <a
              className="dark:text-white dark:hover:text-slate-300 hover:text-slate-700 text-slate-900 cursor-pointer font-bold"
              href={void 0}
          ...
          <li
            className="hover:-translate-y-1 transition-all duration-500"
            onClick={() => setPage(CV_Page.Projects)}
            onKeyDown={() => setPage(CV_Page.Projects)}
          >
            <a
              className="dark:text-white dark:hover:text-slate-300 hover:text-slate-700 text-slate-900 cursor-pointer font-bold"
              href={void 0}
          ...
          <li
            className="hover:-translate-y-1 transition-all duration-500"
            onClick={() => setPage(CV_Page.Contact)}
            onKeyDown={() => setPage(CV_Page.Contact)}
          >
            <a
              className="dark:text-white dark:hover:text-slate-300 hover:text-slate-700 text-slate-900 cursor-pointer font-bold"
              href={void 0}
        ...
        <div
          id="section"
          className="p-2.5 rounded-2xl max-xl:rounded-xl min-h-full h-96"
        >
  ...
  return (
    <section className="p-4">
      ...
      <p className="leading-snug py-1 text-lg">
        Hi! I'm Arthur, a {calcYears(birth)}-year-old self-taught developer from
        Brazil. I started my journey in web development with a focus on backend,
        but I've come to appreciate the creative aspects of frontend as well.
      </p>
      <p className="leading-snug py-1 text-lg">
        Beyond coding, I'm passionate about art in all its forms – music, film,
        visual art, and even video games. While I still enjoy gaming, I find
        myself spending more time nowadays crafting code and building projects.
      </p>
      <p className="leading-snug py-1 text-lg">
        I thrive on challenges and often create my own when none present
        themselves (like{" "}
        <Link
          isExternal
          showAnchorIcon
          href="https://hiarthurbr/redstone"
    ...
    <section className="p-4">
      <div>
        ...
        <ul className="list-disc list-inside text-left leading-snug font-semibold mx-4">
          <li>
            {js_years} {js_text} experience in TypeScript/JavaScript
          </li>
          <li>
            {react_years} {react_text} experience in React
          </li>
          <li>
            {nextjs_years} {nextjs_text} experience in Next.js
          </li>
          <li>
            {rust_years} {rust_text} experience in Rust
          </li>
          <li>
            {cpp_years} {cpp_text} experience in C++
          </li>
          <li>
            {python_years} {python_text} experience in Python
          </li>
          <li>
            {linux_years} {linux_text} experience in Linux server
          </li>
          <li>
            {aws_years} {aws_text} experience in AWS
          </li>
        </ul>
      </div>
    </section>
  ...
  return (
    <section className="p-4">
      <div className="">
        ...
        <p className="leading-tight text-justify mx-4">
          Systems Development Engineering Certificate at ETEC Rosa Perrone
          Scavone (2020-2023)
        </p>
      </div>
      <div>
        ...
        <ul className="list-disc list-inside text-justify leading-snug mx-4">
          ...
          <li>
            <Link
              isExternal
              showAnchorIcon
              href="https://premiacao.obmep.org.br/17obmep/verRelatorioPremiadosMencao-SP.3.do.htm"
              target="_blank"
              rel="noopener noreferrer"
              className="text-white hover:text-slate-300 dark:hover:text-slate-400 transition-all duration-500 hover:underline font-semibold"
            >
              Honorable mention medal in the Brazilian Mathematical Olympiad of
              Public Schools (OBMEP) 2022
            </Link>
          </li>
          <li>
            <Link
              isExternal
              showAnchorIcon
              href="http://www.oba.org.br/site/index.php/zerouminforma.com.br/estudante-da-escola-do-sesi-de-dourados-recebe-medalha-de-ouro-na-olimpiada-brasileira-de-astronomia-e-astronautica/?p=conteudo&idcat=22&pag=conteudo&acao=mostra&idaluno=224983&olimp=oba&ed=2022"
              target="_blank"
              rel="noopener noreferrer"
              className="text-white hover:text-slate-300 dark:hover:text-slate-400 transition-all duration-500 hover:underline font-semibold"
            >
              Silver medal in the Brazilian Olympiad of Astronomy and
              Astronautics (OBA) 2022
            </Link>
          </li>
          <li>
            <Link
              isExternal
              showAnchorIcon
              href="https://drive.google.com/file/d/15Ec4AwZR9edky3y6SWlPDfFGXQbDUygY/view?usp=sharing"
              target="_blank"
              rel="noopener noreferrer"
              className="text-white hover:text-slate-300 dark:hover:text-slate-400 transition-all duration-500 hover:underline font-semibold"
            >
          ...
          <li>
            <Link
              isExternal
              showAnchorIcon
              href="http://www.robotica.cpscetec.com.br/verEventos.php?pag=54"
              target="_blank"
              rel="noopener noreferrer"
              className="text-white hover:text-slate-300 dark:hover:text-slate-400 transition-all duration-500 hover:underline font-semibold"
            >
    ...
    <section className="p-4">
      ...
      <div className="grid grid-flow-row grid-cols-2 max-sm:grid-cols-1 gap-8 justify-evenly items-center max-sm:mx-12">
        ...
        <Card className="w-full h-full">
          <CardHeader>
            <p>
              <span className="text-default-300">hiarthurbr/</span>
          ...
          <CardBody>
            <p className="leading-snug">
              Minecraft Protocol implementation in Rust.
            </p>
          </CardBody>
          <Divider />
          <CardFooter>
            <Link
              isExternal
              showAnchorIcon
              href="https://github.com/hiarthurbr/redstone"
            >
              Visit project on GitHub.
            </Link>
          </CardFooter>
        ...
        <Card className="w-full h-full">
          ...
          <CardBody>
            <p className="leading-snug">
              The ultimate remote Minecraft server management tool.
            </p>
          </CardBody>
          <Divider />
          <CardFooter>
            <Link
              isExternal
              showAnchorIcon
              href="https://github.com/hiarthurbr/mcmngr"
            >
              Visit project on GitHub.
            </Link>
          </CardFooter>
        ...
        <Card className="w-full h-full">
          ...
          <CardBody>
            <p className="leading-snug">
              Lifesaver functions & constants in multiple languages that I write
              over time.
            </p>
          </CardBody>
          <Divider />
          <CardFooter>
            <Link
              isExternal
              showAnchorIcon
              href="https://github.com/hiarthurbr/utils"
            >
              Visit project on GitHub.
            </Link>
          </CardFooter>
      ...
      <h3 className="py-4 font-bold">
        ...
        <Link isExternal showAnchorIcon href="https://github.com/hiarthurbr">
          GitHub
        </Link>
      </h3>
    </section>
  ...
  return (
    <section className="p-4 text-justify mx-8">
      ...
      <p className="leading-snug">
        ...
        <Link
          className="dark:text-cyan-600 select-text"
          showAnchorIcon
          anchorIcon={<Anchor />}
          href="tel:+55 11 95478-2329"
        >
          +55 11 95478-2329
        </Link>
      </p>
      <p className="leading-snug">
        ...
        <Link
          ...
          showAnchorIcon
          anchorIcon={<Anchor />}
          href="mailto:contact@arthurbr.me"
        >
          contact@arthurbr.me
        </Link>
      </p>
      <p className="leading-snug">
        ...
        <Link
          ...
          isExternal
          showAnchorIcon
          href="https://github.com/hiarthurbr"
        >
          github.com/hiarthurbr
        </Link>
      </p>
      <p className="leading-snug">
        ...
        <Link
          ...
          isExternal
          showAnchorIcon
          href="https://www.linkedin.com/in/ArthurBR"
        >
          linkedin.com/in/ArthurBR
        </Link>
      </p>
    </section>
  );
