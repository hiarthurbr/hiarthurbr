...
import type {
  ResumeLink,
  ResumeLinkIndex as Summary,
  Resume as SummaryPage,
} from "global";
...
async function scrapePages(
  urls: string[],
): Promise<PromiseSettledResult<SummaryPage>[]> {
  return await Promise.allSettled(
    urls.map(async (url) => {
      ...
      const links = await Promise.all(
        Array.from(await page.$$(link_selector)).map(
          async (link) => await (await link.getProperty("href")).jsonValue(),
        ),
      );
      links.shift();
      const title = (await page.title())
        .replace(" | resumoporcapítulo", "")
        .trim();

      const chapters = await page.$$eval(chapter_selector, (chapters) => {
        const query = chapters.filter(
          (el) =>
            el.parentElement?.parentElement?.previousElementSibling?.getAttribute(
              "type",
            ) === "empty-line" &&
            el?.parentElement?.parentElement?.nextElementSibling
              ?.nextElementSibling?.textContent !== "Deixe seu comentário:",
        );
      ...
      const get_links = links.map(
        (link, index) =>
          [link, index, false as boolean] satisfies [string, number, boolean],
      );
      while (texts.length < links.length) {
        const results = await Promise.allSettled(
          get_links
            ...
            .map(async ([link, index]) => {
              ...
              const chapter_title = await page.$$eval(
                title_selector,
                (titles) => titles.pop()!.textContent!,
              );
              const text = await page.$$eval(text_selector, (texts) =>
        ...
        results
          ...
          .forEach((result) => {
            const index = get_links.findIndex(
...
async function main() {
  ...
  while (Cache.length < base_pages.length) {
    ...
    build.forEach((page, i) => {
      if (page.status === "fulfilled") {
        ...
        const width =
          cache_link_pages[i]?.length ?? 0 > error?.length
            ? cache_link_pages[i]?.length
            : error?.length;
        const error_padding =
          error?.length < width ? " ".repeat(width - error?.length) : "";
        console.error(
  ...
  const base_json = {
    "per-chapter": ["Por capítulo", [], false],
  ...
  try {
    Cache.forEach((file) => {
      ...
      writeFileSync(
        `${base_write_chapters}/${title_hash}.json`,
        JSON.stringify(file),
      );
      console.info(
        `Wrote ${file.title} to ${base_write_chapters}/${title_hash}.json`,
      );
    });
